#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# train_xgb2.py — XGBoost для бинарной классификации injury/no-injury (NPY → агрегированные фичи)
...
def choose_threshold_constrained(
    y_true, proba,
    min_recall1=0.93,
    min_precision1=None,        # если None — проверяем только recall
    prefer="tnr",               # "tnr" | "f1" | "balacc"
    warn=True,
    compromise_weights=(10.0, 5.0, 1.0, 0.2)  # штрафы/награды: (rec-short, prec-short, +tnr, +rec)
):
    """
    ...
    # --- Лучший компромисс, если требований достичь нельзя
...
def train_xgb(
    ...
    if fixed_thr is not None:
        ...
        # режимы: "tnr_constrained" (жёсткие ограничения) или "f1" (только recall-констрейнт)
    ...
    # лог + сохранение инфы о пороге
    print(f"[thr] prefer={thr_info.get('feasible', False)} feasible={thr_info.get('feasible')} | "
          f"thr={thr:.3f} | TNR={thr_info.get('tnr', float('nan')):.3f} | "
          f"R1={thr_info.get('recall1', float('nan')):.3f} | P1={thr_info.get('precision1', float('nan')):.3f}")
...
if __name__ == "__main__":
    ...
    p.add_argument("--min_precision1", type=float, default=0.95, help="ограничение на precision класса 1 (используется при tnr_constrained)")
    ...
    p.add_argument("--thr_prefer", type=str, default="tnr", choices=["tnr","f1","balacc"],
                   help="что максимизировать среди допустимых порогов в режиме tnr_constrained")
