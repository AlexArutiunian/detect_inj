...
# ===================== detect_inj/train3_tpu.py =====================
# Добавлена поддержка TPU (TPUStrategy + mixed-precision bfloat16).
# Для запуска на TPU:  --device tpu  (и при необходимости --tpu_address grpc://10.0.0.2:8470)
...
def train_deep(name, train_seq, dev_seq, test_seq, class_weight, epochs, out_dir, input_shape, strategy, using_tpu):
    k = _lazy_tf()
    tf = k["tf"]

    # строим модель внутри scope (важно для TPU)
    with strategy.scope():
    ...
    # для TPU Keras требует steps_per_epoch/validation_steps при работе с Sequence
    steps_per_epoch = len(train_seq)
    validation_steps = len(dev_seq)
    ...
    # сохранение: на TPU policy mixed_bfloat16 -> веса в float32 сохраняются корректно
...
if __name__ == "__main__":
    ...
    parser.add_argument("--device", type=str, default="auto", choices=["auto","cpu","gpu","tpu"],
                        help="Где обучать нейросеть. Для классики игнорируется.")
    parser.add_argument("--tpu_address", type=str, default="", help="grpc://… если нужно явно указать TPU")
    ...
    # ========= ВЕТКА НЕЙРОСЕТЕЙ =========
        ...
        # ====== стратегия устройств (TPU/GPU/CPU) ======
        strategy, using_tpu = setup_strategy(args.device, args.tpu_address)
        ...
        dev_metrics, test_metrics, thr, model = train_deep(
            ...
            strategy=strategy, using_tpu=using_tpu
        )
